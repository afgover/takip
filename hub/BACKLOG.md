...
- [x] B-103 · (agent+user) SEC-012: fine-grained token'ın kapsadığı repo sayısı
...
- [x] B-130 · (agent) `task_lifecycle_test`'in iki testi 10 dakika zaman aşımına
      ...
      **Bu oturumda bulundu, kaynağı bu oturum değildi.** Ölçüldü:
      ...
      `_busy` true olurken `CircularProgressIndicator` çiziliyor
      ([add_task_screen.dart:211](../lib/features/add_task/add_task_screen.dart)),
      o da **sonsuz** bir animasyon; `pumpAndSettle` animasyon bitene kadar
      pompaladığı için hiç dönmüyor. Testin kendi yorumu zaten "pumpAndSettle
      yetmiyor" diyor ve `pollAndSettle`'ı bu yüzden yazmış — ama gönderim
