...
session: none
type: info
title: "Project Taskr — Claude Code Rules"
created: 2026-08-01T00:00:00Z
---

> **Arşiv.** Kaynak: `taskr@project-taskr:CLAUDE.md` — 2026-08-01'de
...
# Project Taskr — Claude Code Rules

For every Project Taskr coding, API, deployment, task, agent, session, or project-management request,
load and follow `.codex/skills/project-taskr-agent-api/SKILL.md` and its `references/`.

- Work only on the `project-taskr` branch/product boundary. Never mix Taskr Basic (`main`) changes.
- Never place an agent key, JWT, password, session code, or other secret in Git, logs, events,
  metadata, prompts, or chat output.
- Use the Project Taskr API as the durable record for agent work: ensure a session, record every
  prompt as a task, log answer turns, tests, Git metadata, follow-ups, approvals, and final status.
- Agents submit work; only the prompt owner or assigned reviewer performs final acceptance.

The canonical operating directive below is served by the API (`GET /agent/bootstrap`) and kept in sync
by `npm --prefix backend run directive:sync`. Do not edit inside the managed block by hand.
